...
import {
  ...
  PlusCircle, Calendar, User, Info, RefreshCw, Loader2
} from "lucide-react";
...
export default function Workflow() {
  ...
  const fetchWorkflow = async () => {
    try {
      ...
      const list = workflowsRes.data?.data || [];
      if (list.length > 0) {
        const wfId = list[0].id;
        const detailRes = await workflowAPI.getById(wfId);
        const wf = detailRes.data?.data;
        if (wf) {
          setCurrentWf(wf);
          const uiSteps = [
            { id: 1, label: "HR Verification", icon: UserCheck, status: wf.hr_verification_status || "completed", desc: "Initial profile and eligibility check", completedBy: wf.hr_verified_by, completedAt: wf.hr_verified_at, remarks: wf.hr_remarks },
            { id: 2, label: "Document Upload", icon: FileText, status: wf.document_upload_status || "completed", desc: "Employee uploads required documents", completedAt: wf.documents_uploaded_at },
            { id: 3, label: "AI Verification", icon: Cpu, status: wf.ai_verification_status || "active", desc: "AI validates document authenticity", remarks: wf.ai_remarks },
            { id: 4, label: "Manager Approval", icon: UserCheck, status: wf.manager_approval_status || "pending", desc: "Department manager reviews and approves", remarks: wf.manager_remarks },
            { id: 5, label: "IT Setup", icon: Monitor, status: wf.it_setup_status || "pending", desc: "Laptop, accounts, and tools provisioned", remarks: wf.it_remarks },
            { id: 6, label: "Orientation", icon: BookOpen, status: wf.orientation_status || "pending", desc: "Company culture and policy orientation", remarks: wf.orientation_remarks },
            { id: 7, label: "Completed", icon: CheckCircle2, status: wf.workflow_status === "Completed" ? "completed" : wf.workflow_status === "Pending Automation" ? "pending" : "active", desc: "Onboarding process finalized" }
          ];
          setStepsState(uiSteps);
          const activeIndex = uiSteps.findIndex(s => s.status === "active");
          if (activeIndex !== -1) {
            setSelectedId(uiSteps[activeIndex].id);
          } else {
            const pendingIndex = uiSteps.findIndex(s => s.status === "pending");
            if (pendingIndex !== -1) {
              setSelectedId(uiSteps[pendingIndex].id);
            }
          }
        }
      }
    } catch (err) {
      console.warn("Using fallback mock workflow data:", err);
    } finally {
      setLoading(false);
    }
  };

  const handleRetryAutomation = async (wfId) => {
    setRetrying(true);
    try {
      await workflowAPI.retryAutomation(wfId);
      showToast("n8n onboarding automation triggered successfully!", "success");
      fetchWorkflow();
    } catch (err) {
      showToast("n8n automation is still offline.", "error");
    } finally {
      setRetrying(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-6">
      ...
      <motion.div variants={containerVariants} initial="hidden" animate="visible" className="max-w-6xl mx-auto">
        ...
        {currentWf?.workflow_status === "Pending Automation" && (
          <motion.div
            variants={itemVariants}
            className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-6 flex flex-wrap items-center justify-between gap-4 shadow-sm"
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-bold text-amber-800 text-sm">n8n Automation Pending</h3>
                <p className="text-xs text-amber-600 mt-0.5">The backend was unable to reach n8n during registration. Retrying will execute the simulated department sequence.</p>
              </div>
            </div>
            <button
              onClick={() => handleRetryAutomation(currentWf.id)}
              disabled={retrying}
              className="flex items-center gap-1.5 bg-amber-600 hover:bg-amber-700 text-white px-4 py-2 rounded-lg text-sm font-semibold transition-colors disabled:opacity-50"
            >
              {retrying ? (
                <>
