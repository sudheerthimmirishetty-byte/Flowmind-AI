...
export default function Profile() {
  ...
  const initialProfile = {
    fullName:       user?.name || user?.full_name || 'Alex Mitchell',
    email:          user?.email || 'employee@company.com',
    phone:          user?.phone || '+91-9876543210',
    dob:            user?.dob || '1998-03-15',
    gender:         user?.gender || 'Male',
    address:        user?.address || '204, Green Valley Apartments, Whitefield, Bengaluru – 560066',
    employeeId:     user?.employee_id || user?.id?.slice(0, 8) || 'EMP042',
    department:     user?.department || 'Engineering',
    role:           user?.role_title || user?.role || 'Software Engineer',
    manager:        user?.manager_name || 'Sarah Johnson',
    joiningDate:    user?.joining_date || '2026-07-01',
  ...
  const handleProfileSave = async () => {
    ...
    try {
      ...
      setErrorProfile(err?.response?.data?.message || err.message || 'Failed to update profile')
    } finally {
