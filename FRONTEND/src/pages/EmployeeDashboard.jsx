        ...
        <motion.div variants={fadeUp}>
          <Card className="h-full">
            ...
            <div className="space-y-3">
               {[
                ['Employee ID', user?.employee_id || user?.id?.slice(0, 8) || 'EMP042'],
                ['Department', user?.department || 'Engineering'],
                ['Role', user?.role_title || user?.role || 'New Hire'],
                ['Manager', user?.manager_name || 'HR Manager'],
                ['Joining Date', user?.joining_date ? new Date(user.joining_date).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' }) : 'July 1, 2026'],
                ['Employment Type', user?.employment_type || 'Full-Time'],
              ].map(([l, v]) => (
