...
import { generateMockEmployees, getInitials, DEPARTMENTS, cn } from "../utils/helpers";
...
export default function EmployeeList() {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      setAllEmployees(generateMockEmployees(20));
      setLoading(false);
    }, 800);
  ...
  const departments = useMemo(() => {
    const deps = [...new Set(allEmployees.map((e) => e.department))].sort();
    return ["All", ...deps];
  }, [allEmployees]);

  const filtered = useMemo(() => {
    let data = allEmployees;
    if (search)       data = data.filter((e) => [e.name, e.id, e.email].some((v) => v?.toLowerCase().includes(search.toLowerCase())));
    ...
    data = [...data].sort((a, b) => {
      ...
      if (sortKey === "progress") { av = Number(av); bv = Number(bv); }
      if (av < bv) return sortDir === "asc" ? -1 : 1;
      if (av > bv) return sortDir === "asc" ? 1 : -1;
      return 0;
    });
    return data;
  }, [allEmployees, search, deptFilter, statusFilter, sortKey, sortDir]);
  ...
  const handleDelete = () => {
    setAllEmployees((prev) => prev.filter((e) => e.id !== deleteEmp.id));
    showToast(`${deleteEmp.name} removed from directory.`, "error");
    setDeleteEmp(null);
  };
      ...
      <motion.div variants={containerVariants} initial="hidden" animate="visible" className="max-w-7xl mx-auto">
        ...
        <motion.div variants={itemVariants} className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-slate-500 ml-4">{allEmployees.length} employees in the system</p>
          </div>
        ...
        <motion.div variants={itemVariants} className="bg-white rounded-xl border border-slate-100 p-4 mb-5 shadow-sm">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="ml-auto flex items-center gap-1.5 text-xs text-slate-400">
              <SlidersHorizontal size={13}/> {filtered.length} result{filtered.length !== 1 ? "s" : ""}
            </div>
          </div>
        </motion.div>
          ...
          {!loading && filtered.length > 0 && (
            <div className="flex flex-col sm:flex-row items-center justify-between gap-3 px-5 py-4 border-t border-slate-100 bg-slate-50/50">
              <p className="text-xs text-slate-400">
                Showing <span className="font-semibold text-slate-600">{(page-1)*PAGE_SIZE+1}</span>–<span className="font-semibold text-slate-600">{Math.min(page*PAGE_SIZE,filtered.length)}</span> of <span className="font-semibold text-slate-600">{filtered.length}</span> employees
              </p>
